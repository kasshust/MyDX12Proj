...
bool App::InitIMGUI()
{
    ...
    ID3D12DescriptorHeap* srvheap               = m_pPool[POOL_TYPE_RES]->GetHeap();
    D3D12_CPU_DESCRIPTOR_HANDLE  chandle        = srvheap->GetCPUDescriptorHandleForHeapStart();
    D3D12_GPU_DESCRIPTOR_HANDLE  ghandle        = srvheap->GetGPUDescriptorHandleForHeapStart();

    // return true;
    return ImGui_ImplDX12_Init(device, frameCount, format, srvheap, chandle, ghandle);
}
